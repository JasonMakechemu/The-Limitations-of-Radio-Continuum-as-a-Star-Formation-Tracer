...
'''
Abell 33 is no cluster

import os
from scipy.stats import median_abs_deviation
from sklearn.mixture import GaussianMixture

# Define the list of your galaxy clusters
clusters = [
    'Abell_133', 'Abell_194', 'Abell_209', 'Abell_22',
    'Abell_2485', 'Abell_2597', 'Abell_2645', 'Abell_2667', 'Abell_2744', 'Abell_2751',
    'Abell_2811', 'Abell_2895', 'Abell_3365', 'Abell_3376', 'Abell_3558',
...
'''
MCXC - MCXC Meta-Catalog of X-Ray Detected Clusters of Galaxies - r500 in Mpc: -
    A2485, 2597, 2645, 2667, 2744, 2751, 2811, 2895, 3376, 3558, 3562,
    3667, 4038, J0014.3-6604
    
Radio halos in a mass-selected sample of 75 galaxy clusters - Cuciti et al. -
r500 in Mpc A209, 521

Investigating the outskirts of Abell 133 with Suzaku and Chandra observations -
r500 in A133

DES to HSC: Detecting low-surface-brightness galaxies in the Abell 194 cluster
using transfer learning - r200 A194 0.9824

Weak lensing mass reconstruction of the galaxy cluster
Abell 209 - r200 

Arnaud et. al 2010 Abell 22 r500


Free-form Lens Model and Mass Estimation of the High-redshift Galaxy Cluster 
ACT-CL J0102-4915, “El Gordo” r200 from m200 converted to r500

Environmental Effects on Galaxy Evolution. II. Quantifying the Tidal Features in
 NIR Images of the Cluster Abell 85 r500
 
 abell 545 r500 - https://watermark.silverchair.com/stw2250.pdf?token=AQECAHi208BE49Ooan9kkhW_Ercy7Dm3ZL_9Cf3qfKAc485ysgAAA1IwggNOBgkqhkiG9w0BBwagggM_MIIDOwIBADCCAzQGCSqGSIb3DQEHATAeBglghkgBZQMEAS4wEQQM0qU3xUHUZQkeMadsAgEQgIIDBZktaFkXdiyROZoYbVRvnLGj8-9kS5I-Y3aP2sxXOMLi1881q95TwUJ-hK4QPwxyDbMLxqB9FMz0bicdDnXWPnWcZdC-neWPBC3Z5uJIe3cSIemp2931hrML5JZKGTgv-wfsfl0Bc2yZk5PUIhxNHGIkysdKW_L5OIkWwhJM6k6UoV5M3LNWgb4rvrNJ4tOTl7hEXwDt8cRmTyNfdnXwjsf1AA59b2FyC1VJpr5_I2FNL1_9V4V4YUBGZkAySIdxD_5tqME6BlcaXS6h9-L-X8TeBH5IWhURWVREMSl405_Dc4OlP39XZ6EFSKOMbG5DoNgI18pHILGe2KxXaTSZiLZ0d6jMf15tKOuYxYyHMNAL2GxxF2XLLi9bl2K4424UDORz1AIzDZzx2bUiG7ltxhhcb6IQdkztFv5oNSyinOTkiVpKv4VFyzRiCOC7MV1Gt9xcEwg1J5-IQAYL6_kiEa2utosPkTPpXiUinDwKTWpwwdJw-6W0W3rXWKjPlxYLtqmJjcxHhheq-Im3sCX_0EtuX1sEe-kC3emg2MRGEGSKmsidC6qNObK1ydkyfJ_PaKVj4H0hN3wxukB21gVqM6ZupX9haoNFaIGkixTE6Zcs346_3XVOAXPnMoo626Qrd3tprc4GeHKY5zLMVIsfxJ0AAjjRbSWmpUuALJ--BBJYNG4y9AaMagULvj-Mv5J8eylgV0Z5H4wjaGQbknHS7_4naQjVwcYxb3q5ew0Pf0r_qKvcG2bMiNx1r7doMD6OKqXwGwdzG9C6RXYJppLahi7BMel3kjfDsS0z6PktXML8RW_APtdmTlifsFMJ_g9t4Ll8HkS04JNZKNiqpYg3FsT-ziWCnHoIaOViYpBhl5FjZyFqJDHCy39pe3DmS-yS_NvJ9hiUqvrCsmbTMB_L2dzPYyarLYaSSTnGIsz3X5IsmzUA8xl8A0xm48o3tSEW5crMGJI25EPyHE3VELGP5EWA-lAuV2MKYiUtFLI5x1p6ufZQDygjpyQYzNuzjZUfs2Vwmd9e
 
 Line-of-sight Elongation and Hydrostatic Mass Bias of the Frontier
 Fields Galaxy Cluster Abell 370 r200
 
 X-ray study of Abell 3365 with XMM-Newton - r500
'''  
'''
r500 = [0.937, 0.9824/1.5, 1.330/1.5, 1.056, 1.0622, 1.1066, 1.0897, 1.3031, 1.2359, 0.7431, 
        1.0355, 1.0896, 0.81, 0.8726, 1.101, 0.9265, 1.199, 1.64/1.5, 0.8863,
        1.240, 1.35, 1.2, 6.8, 1.0117]


# Directory where your flux ratio CSVs are located
csv_directory = './'

exclude_outliers = False  # Toggle this to include/exclude 2-sigma SFR ratio outliers

# ---- Step 1: Initialize storage before the loop (if not already done) ----
cluster_breakpoints = []

for cluster in clusters:
    print(f"\n📊 Processing cluster: {cluster}")
    file_name = f"{cluster}_flux_ratio.csv"
    file_path = os.path.join(csv_directory, file_name)

    if not os.path.exists(file_path):
        print(f"❌ Skipping {cluster} — File not found: {file_path}")
        continue

    try:
        df = pd.read_csv(file_path)
    except Exception as e:
        print(f"❌ Skipping {cluster} — Error reading CSV: {e}")
        continue

    required_cols = [
        'observed_flux_in_cluster',
        'predicted_flux_in_cluster',
        'flux_ratio',
        'SFR_Msun_per_yr',
    ...
    if not all(col in df.columns for col in required_cols):
        print(f"❌ Skipping {cluster} — Missing required columns.")
        continue

    data_for_gmm = df[required_cols].dropna()

    if data_for_gmm.empty:
        print(f"⚠️ Skipping {cluster} — No valid data points after dropping NaNs.")
        continue

    # Compute SFR ratio and outlier mask
    gmm_sfr_ratio = data_for_gmm['Predicted_SFR_Msun_per_yr_from_L'] / data_for_gmm['SFR_Msun_per_yr']
    ...
    if exclude_outliers:
        data_for_gmm_filtered = data_for_gmm.loc[~outlier_mask].copy()
        gmm_sfr_ratio_filtered = gmm_sfr_ratio.loc[~outlier_mask].copy()
    else:
        data_for_gmm_filtered = data_for_gmm.copy()
        gmm_sfr_ratio_filtered = gmm_sfr_ratio.copy()

    if data_for_gmm_filtered.empty:
        print(f"⚠️ Skipping {cluster} — No valid data points after excluding outliers.")
        continue

    # --- Spearman correlation binning ---
    ...
    flux = data_for_gmm_filtered['observed_flux_in_cluster'].values
    log_flux = np.log10(flux + 1e-5)
    ratio = data_for_gmm_filtered['flux_ratio'].values
    ...
    X_for_gmm = np.log10(data_for_gmm_filtered[['observed_flux_in_cluster', 'flux_ratio']].values + 1e-5)
    ...
    print(f"✅ GMM applied for {cluster}. Found {gmm.n_components} components.")

    # Consistent labeling of clusters by mean flux_ratio
    mean_flux_ratio_comp0 = data_for_gmm_filtered['flux_ratio'][labels == 1].mean()
    mean_flux_ratio_comp1 = data_for_gmm_filtered['flux_ratio'][labels == 0].mean()
    ...
    # Append result
    cluster_breakpoints.append({
        'cluster': cluster,
        'gmm_breakpoint': gmm_breakpoint,
        'spearman_breakpoint': spearman_breakpoint,
        'max_rho': max_rho,
        'combined_pval': combined_pval  # Ensure this variable is defined
    })
    ...
    print(f"Combined Fisher p-value for cluster {cluster}: {combined_pval:.3e}")
    print(f"Spearman p-values for each bin in cluster {cluster}:")
    ...
    # Set log-log scales and labels before plotting
    ax1.set_xscale('log')
    ax1.set_yscale('log')
    ax1.set_xlabel('Observed Flux Density [mJy]')
    ax1.set_ylabel('Flux Density Ratio (Predicted/Observed)')
    ax1.set_title(f'GMM Clustering — {cluster}')
    ax1.grid(True, which="both", ls="-", alpha=0.2)
    
    if exclude_outliers:
        # Plot only the non-outliers
        scatter = ax1.scatter(
            data_for_gmm.loc[~outlier_mask, 'observed_flux_in_cluster'],
            data_for_gmm.loc[~outlier_mask, 'flux_ratio'],
            c=consistent_labels,
            cmap='coolwarm',
            alpha=0.7,
            marker='o',
        ...
        scatter2 = ax1.scatter(
            data_for_gmm.loc[outlier_mask, 'observed_flux_in_cluster'],
            data_for_gmm.loc[outlier_mask, 'flux_ratio'],
            c=consistent_labels[outlier_mask == True],
            cmap='coolwarm',
            alpha=0.7,
            marker='^',
            label='>2σ SFR Ratio'
    ...
    ax2.set_xlabel("Observed Flux Density [mJy]")
    ax2.set_ylabel("Spearman Correlation (Obs. Flux Density vs. Flux Ratio)", fontsize=8)
    ax2.set_title(f"Spearman Correlation: {cluster}")
    ax2.grid(True)
    ax2.set_xscale('log')
    ...
    plt.tight_layout()
    plt.savefig(f'gmm_spearman_combined_{cluster}.png', dpi=300)
    plt.close()

    print(f"📊 Saved 2-panel plot for {cluster}: GMM + Spearman")
